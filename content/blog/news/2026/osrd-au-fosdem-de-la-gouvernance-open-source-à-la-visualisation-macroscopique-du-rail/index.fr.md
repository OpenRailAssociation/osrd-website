...
OSRD avait besoin d’une visualisation macroscopique puissante, sans complexifier son architecture légère (Rust, Python, React). L’intégration de NGE s’est donc faite en mode “standalone” : seule la couche graphique est utilisée, encapsulée dans un Web Component ou un <iframe>, sans embarquer le back-end Java de NGE. 
...
* Créer un élément HTML personnalisé (<sbb-root>) 
* L’intégrer dans l’interface OSRD (via <iframe> ou dans le DOM) 
...
Une collaboration qui fait avancer tout l’écosystème ferroviaire 
