...
* `./scripts/pr-tests-compose.sh 8914 down`: Shuts down the test application instance for PR #8914.
* `./scripts/pr-tests-compose.sh 8914 down-and-clean`: Shuts down the test instance and cleans all the instance's docker volumes (PG data, Redis cache, RabbitMQ) to prevent any side-effects.
